...
def relative_density(df,which="baldi et al_1986"):
    ...
    if which == "baldi et al_1986":
        ...
        t = 1000     # Aging factor (Pleistocene sands = ~13,000 years)
        pa = 100     # Reference pressure [kPa]
        Qtn = (df.qt/(pa/1000))/(df.sig_eff/pa)**0.5        # Normalised CPT resistance, corrected for overburden pressure
        ...
        raise ValueError(f"The relative density method {which} is not recognised.")
    # Exclude clay
    # df = Isbt(df)
...
def G0(df,method="constant alpha", alpha=5.77,v=0.2,K_g = 300):
    ...
    if method == "constant alpha":
        ...
        E = 0.015*(10**(0.55*df.Ic+1.68))*(df.qt-df.sig/1000)
        df["G0"] = E/(2*(1+v))
...
def su(df,Nkt = 14):
    if "sig" not in df.columns:
        raise ValueError("Insert the total stress sig into dataframe")
    
    df["su"] = (df.qt*1000 - df.sig)/Nkt    # kPa
